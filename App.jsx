...
export default function WeeklyTimesheetCalculator() {
  ...
  async function runOcrForActiveAttachment() {
    ...
    try {
      ...
      const contentBlock = activeAttachment.isPdf
        ? { type: "document", source: { type: "base64", media_type: "application/pdf", data: base64 } }
        : { type: "image", source: { type: "base64", media_type: activeAttachment.mimeType || "image/jpeg", data: base64 } };

      const promptText = `この画像は手書きの出勤簿です。次のJSONオブジェクトのみを出力してください(説明文やコードブロック記号は不要、JSON以外の文字は一切出力しないこと)。
...
- 出力はJSONオブジェクトのみとし、前後に説明文やマークダウンのコードブロックを付けないこと。`;
      ...
      if (!response.ok || data.type === "error") {
        const apiMsg = (data.error && data.error.message) || `HTTP ${response.status}`;
      ...
      if (jsonStart === -1 || jsonEnd === -1) {
        throw new Error("AIの応答からJSONを検出できませんでした。もう一度お試しください。");
      }
      ...
      try {
        ...
        throw new Error("AIの応答を解析できませんでした(出力が途中で切れた可能性があります)。もう一度お試しください。");
      }
      ...
      setOcrStatus("error");
      setOcrMessage(`読み取りに失敗しました: ${err && err.message ? err.message : "不明なエラー"}`);
    }
  }
